...
@implementation FixRecordViewModel
...
-(id)init
{
    ...
    if (self) {
        ...
        self.fixRecordList = [[NSMutableArray alloc]init];
        
        self.fixRecordModel = [[FixRecordModel alloc]init];
...
-(RACSignal *)feedDataWithType:(LoadType)loadType
{
    ...
    return [RACSignal createSignal:^RACDisposable *(id<RACSubscriber> subscriber) {
        ...
        NSDictionary *tempDic = @{
                                  @"code":XS011,
                                  @"serial_no":[NSString stringWithFormat:@"%@%@",[CMUtility currentTimestampMillisecond],XS011_serial_no],
                                  @"errorcode":@"0",
                                  @"errormsg":@"success",
        ...
        @weakify(self)
        [SocketIO_Singleton sendEmit:XS011 withMessage:jsonStr];
        SocketIO_Singleton.xr011CallBackResult = ^(NSDictionary *resultDict){
            @strongify(self)
            ...
            if ([errorcode isEqualToString:SUCCESS_CODE]) {
                ...
                if (loadType == LoadData) {
                    [self.fixRecordList removeAllObjects];
                }
                
                [self.fixRecordList addObjectsFromArray:(NSMutableArray *)[MTLJSONAdapter modelsOfClass:[FixRecordModel class] fromJSONArray:resultDict[@"datas"] error:nil]];
                
                [subscriber sendNext: self.fixRecordList];
