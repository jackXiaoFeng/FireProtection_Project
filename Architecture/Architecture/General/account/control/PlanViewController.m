...
@property (nonatomic, strong) PlanViewModel *viewModel;
...
- (void)viewDidLoad {
    ...
    //添加刷新
    self.viewModel  = [[PlanViewModel alloc]init];
    //首次刷新数据
...
- (void)headerWithRefreshing
{
    ...
    [[[[self.viewModel feedDataWithType:LoadData] map:^id(NSDictionary * dic) {
        ...
        return self.viewModel.planList;
        ...
        BOOL hidden;
        if (self.viewModel.planList.count == 0) {
            hidden = NO;
        }else
        {
...
- (UIView *) tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    NSArray *nameArray = @[@"设备名称编号",@"地点",@"巡检周期"];
    
    NSArray *widthArray = @[
                            @(DEF_DEVICE_SCLE_WIDTH(272)),
                            @(DEF_DEVICE_SCLE_WIDTH(198)),
                            @(DEF_DEVICE_SCLE_WIDTH(282))];
    NSArray *xArray = @[@0,
                        @(DEF_DEVICE_SCLE_WIDTH(272)),
                        @(DEF_DEVICE_SCLE_WIDTH(272)+DEF_DEVICE_SCLE_WIDTH(198))
                        ];
...
{
    return [PlanTableViewCell PlanCellHeight];
}

-(NSInteger )tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    return self.viewModel.planList.count;
}

- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    static NSString *cellIdentify = @"PlanCellIdentify";
    PlanTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIdentify];
    if (!cell) {
        cell = [[PlanTableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellIdentify];
        cell.backgroundColor = [UIColor whiteColor];
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
    }
    
    if (self.viewModel.planList.count > 0) {
        cell.planModel  = self.viewModel.planList[indexPath.row];
        cell.hidenLine= (indexPath.row== self.viewModel.planList.count-1); //通过组模型数组来拿到每组最后一行
    }
