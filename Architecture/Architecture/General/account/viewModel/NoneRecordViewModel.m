...
@implementation NoneRecordViewModel
...
-(id)init
{
    ...
    if (self) {
        ...
        self.noneRecordList = [[NSMutableArray alloc]init];
        
        self.noneRecordModel = [[NoneRecordModel alloc]init];
...
-(RACSignal *)feedDataWithType:(LoadType)loadType
{
    ...
    return [RACSignal createSignal:^RACDisposable *(id<RACSubscriber> subscriber) {
        ...
        NSDictionary *tempDic = @{
                                  @"code":XS033,
                                  @"serial_no":[NSString stringWithFormat:@"%@%@",[CMUtility currentTimestampMillisecond],XS033_serial_no],
                                  @"errorcode":@"0",
                                  @"errormsg":@"success",
        ...
        @weakify(self)
        [SocketIO_Singleton sendEmit:XS033 withMessage:jsonStr];
        SocketIO_Singleton.xr033CallBackResult = ^(NSDictionary *resultDict){
            @strongify(self)
            ...
            if ([errorcode isEqualToString:SUCCESS_CODE]) {
                ...
                if (loadType == LoadData) {
                    [self.noneRecordList removeAllObjects];
                }
                
                [self.noneRecordList addObjectsFromArray:(NSMutableArray *)[MTLJSONAdapter modelsOfClass:[NoneRecordModel class] fromJSONArray:resultDict[@"datas"] error:nil]];
                
                [subscriber sendNext: self.noneRecordList];
