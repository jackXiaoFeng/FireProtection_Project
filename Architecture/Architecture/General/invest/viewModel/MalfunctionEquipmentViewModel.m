//
//  MalfunctionEquipmentViewModel.m
//  Architecture
//
...
@interface MalfunctionEquipmentViewModel()
/**
 *  分页
 */
@property int Page;
...
@implementation MalfunctionEquipmentViewModel
...
-(id)init
{
    ...
    if (self) {
        ...
        self.malfunctionList = [[NSMutableArray alloc]init];
        
        self.malfunctionEquipmentModel = [[MalfunctionEquipmentModel alloc]init];
...
-(RACSignal *)feedDataWithType:(LoadType)loadType
{
    ...
    return [RACSignal createSignal:^RACDisposable *(id<RACSubscriber> subscriber) {
        ...
        NSDictionary *tempDic = @{
                                  @"code":XS010,
                                  @"serial_no":[NSString stringWithFormat:@"%@%@",[CMUtility currentTimestamp],XS010_serial_no],
                                  @"errorcode":@"0",
                                  @"errormsg":@"success",
        ...
        SocketIO_Singleton.xr010CallBackResult = ^(NSDictionary *resultDict){
            ...
            if ([errorcode isEqualToString:SUCCESS_CODE]) {
                ...
                if (loadType == LoadData) {
                    [self.malfunctionList removeAllObjects];
                }
                
                [self.malfunctionList addObjectsFromArray:(NSMutableArray *)[MTLJSONAdapter modelsOfClass:[MalfunctionEquipmentModel class] fromJSONArray:resultDict[@"datas"] error:nil]];
                
                [subscriber sendNext: self.malfunctionList];
...
-(RACSignal *)alarmEquipmentMaintenanceWithDegree:(NSString *)degree
{
    ...
    return [RACSignal createSignal:^RACDisposable *(id<RACSubscriber> subscriber) {
        ...
        NSDictionary *tempDic = @{
                                  @"code":XS014,
                                  @"serial_no":[NSString stringWithFormat:@"%@%@",[CMUtility currentTimestamp],XS014_serial_no],
                                  @"errorcode":@"0",
                                  @"errormsg":@"success",
                                  @"token":CMMemberEntity.token,
                                  @"dat":arr                              };
